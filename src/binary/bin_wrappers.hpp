...
void conv2d_wrapper(Layer<T1> & in_layer, Layer<binary16> & out_layer, int slice, bool bias){
    float2bin_conv(in_layer, out_layer, slice, bias);
}

template <typename T1>
void conv2d_wrapper(Layer<T1> & in_layer, Layer<binary16> & out_layer, bool bias){
    float2bin_conv(in_layer, out_layer, bias);
}

void conv2d_wrapper(Layer<binary16> & in_layer, Layer<binary16> & out_layer, bool bias){
    bin2reg_conv_prep(in_layer, out_layer);
    reg2bin_conv(in_layer, out_layer, bias);
}

void fc_wrapper(Layer<binary16> & in_layer, Layer<binary16> & out_layer, bool bias){
...
void fc_output_wrapper(Layer<binary16> & in_layer, Layer<binary16> & out_layer, bool bias){
    bin2reg_fc_prep(in_layer);
    reg2int_fc(in_layer, out_layer);
}
